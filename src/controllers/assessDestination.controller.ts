...
class AssessDestinationController {
    ...
    async getByDestination(req: Request, res: Response, next: NextFunction) {
        try {
            const result = await assessDestinationService.getByDestination(req.params.destinationId);
    ...
    async delete(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const result = await assessDestinationService.deleteOne(traveller_id, destination_id);
            res.status(result.status).json(result);
        } catch (err) {
            next(err);
        }
